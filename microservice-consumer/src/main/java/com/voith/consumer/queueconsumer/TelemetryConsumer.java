...
public class TelemetryConsumer {
    ...
    @KafkaListener(topics = "telemetry_topic", groupId = "voith-consumer-group")
    public void consume(String message) {
        if (isPrintMode) {
            consolePrinterService.printMessage("TelemetryConsumer", message);
        } else {
            log.info("📤 [TelemetryConsumer] Storing message in InfluxDB: {}", message);
            influxDBService.storeData("telemetry", message);
        }
    }
}
